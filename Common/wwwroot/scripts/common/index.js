...
var currentAppliedTheme = DEFAULT_THEME;
var dotnetTooltipRef;

// Resolve theme from current URL and normalize known names
...
// Update link tag, body class, UI controls and notify Blazor/components about theme change
function updateThemeFromUrlAndApply() {
    var themeName = resolveThemeFromUrl();
    if (!themeName) themeName = DEFAULT_THEME;

    // Update theme link (<link id="theme">)
    var ThemeEle = document.getElementById('theme');
    if (ThemeEle) {
        try {
            if (ThemeEle.href && ThemeEle.href.indexOf('cdn.syncfusion.com') !== -1) {
                ThemeEle.href = 'https://cdn.syncfusion.com/blazor/32.1.19/styles/' + themeName + '.css';
            } else {
                ThemeEle.href = '_content/Syncfusion.Blazor.Themes/' + themeName + '.css';
            }
        } catch (e) { }
    }

    // Update body class
    var previous = currentAppliedTheme || DEFAULT_THEME;
    if (previous !== themeName) {
        if (document.body.classList.contains(previous)) {
            try { addCurrentThemeToBody(themeName, previous); } catch (e) { document.body.classList.replace(previous, themeName); }
        } else {
    ...
    // Update any UI state/buttons
    try { updateButtonTheme(); } catch (e) { }

    try {
        if (typeof loadSelectedThemeOnThemeSwitch === 'function') {
            loadSelectedThemeOnThemeSwitch(themeName);
        }
    } catch (e) { }

    // Notify Blazor so it can reload dynamic resources if required
    try {
        if (window.sfBlazorSB && window.sfBlazorSB.dotnetRef) {
            var sampleName = getPagePath();
            window.sfBlazorSB.dotnetRef.invokeMethodAsync('LoadDynamicResourcesDuringBrowserNav', sampleName, true, window.location.href);
        }
    } catch (e) { }
}
...
window.addEventListener('load', function () {
    ...
    if (ThemeEle) {
        ...
        let theme = new URL(window.location.href).searchParams.get("theme") || DEFAULT_THEME;
        theme = theme.toLowerCase();
        if (window.location.href.indexOf('blazor.syncfusion.com') != -1 || window.location.href.indexOf('localhost') != -1) {
            theme = theme === "bootstrap5" ? "bootstrap5.3" : theme === "bootstrap5-dark" ? "bootstrap5.3-dark" :theme;
        }
        if (url.length > 1) {
            if (ThemeEle.href.indexOf("cdn.syncfusion.com") !== -1) {
                ThemeEle.href = 'https://cdn.syncfusion.com/blazor/32.1.19/styles/' + theme + '.css';
            }
            else {
  ...
  if (/theme=/g.test(location.search)) {
    ...
    try { updateThemeFromUrlAndApply(); } catch (e) { }
...
function includeErrorMessage() {
    document.querySelector('.e-pv-page-container').append(document.querySelector('.error-msg-dialog'));
}
...
function isMobileDevice(isRtl) {
    ...
    var isDevice = (isMobile.IsDevice || isMobile.IsIos) ? true : false;
    var sampleContent = document.querySelector('.sample-content');
    if (isDevice && sampleContent) {
...
async function handleMouseDown(event) {
    if (event.type != "touchstart") {
        ...
        buttonId = event.target.closest('.e-pv-e-sign-form-field-property').id;
        let startX = event.clientX;
        let startY = event.clientY;
        let clone = await createCloneElement(buttonId, startX, startY);
        let offsetX = 0, offsetY = 0;
        if (!clone) return;
        dotnetTooltipRef.invokeMethodAsync("UpdateIsDropped", true, buttonId)
        ...
        function onMouseUp(event) {
            ...
            if (clone && !targetElement) {
                dotnetTooltipRef.invokeMethodAsync("UpdateIsDropped", false, null)
            }
            if (clone) {
        ...
        event.preventDefault();
        var clickbuttonId = event.target.closest('.e-pv-e-sign-form-field-property').id;
        dotnetTooltipRef.invokeMethodAsync("UpdateSetMode", true, clickbuttonId);
...
async function createCloneElement(buttonId, startX, startY) {
    let zoomFactor = 1;
    zoomFactor = await dotnetTooltipRef.invokeMethodAsync("GetZoomValue");
    let currentUser = document.getElementsByClassName("name")[0].textContent;
...
function hideToast() {
    const toastElement = document.querySelector('.ai-notification');
    if (toastElement) {
...
window.addEventListener('popstate', function () {
    // When user navigates via forward/back, ensure theme from URL is applied to link, body and components
    try { updateThemeFromUrlAndApply(); } catch (e) { }
    if (window.sfBlazorSB && window.sfBlazorSB.dotnetRef) {
        var sampleName = getPagePath();
        var params = new URL(window.location.href).searchParams;
        var newTheme = resolveThemeFromUrl();
        var isThemeChanged = (newTheme !== currentAppliedTheme);
        window.sfBlazorSB.dotnetRef.invokeMethodAsync('LoadDynamicResourcesDuringBrowserNav', sampleName, isThemeChanged, window.location.href);
    }
});
function loadSelectedThemeOnThemeSwitch(theme) {
    let ThemeEle = document.getElementById('theme');
    var loaded = false;
    var onload = function () {
        if (loaded) return; loaded = true;
        try { window.dispatchEvent(new Event('resize')); } catch (e) { console.error(e); }
        ThemeEle.removeEventListener('load', onload);
    };
    ThemeEle.addEventListener('load', onload);
    if (ThemeEle) {
        ThemeEle.href = '_content/Syncfusion.Blazor.Themes/' + theme + '.css';
    }
}