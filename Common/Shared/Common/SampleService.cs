            ...
            if (!string.IsNullOrEmpty(updatedUrl))
                ...
                if (splittedUrl.Length >= 2)
                {
                    try
                    {
                        ...
                        ControlSampleResult? result = SampleBrowser.SampleList.SelectMany(control =>(control?.Samples ?? Enumerable.Empty<Sample>()).Select(sample => new ControlSampleResult{ Control = control, Sample = sample})).FirstOrDefault(x =>string.Equals(x.Sample?.Url, sampleName, StringComparison.OrdinalIgnoreCase));
                        if (result != null)
                        {
                            this.SampleInfo = result.Sample;
                            this.ComponentName = result.Control?.Name;
