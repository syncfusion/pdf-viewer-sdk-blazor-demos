...
namespace BlazorDemos.Shared
{
    ...
    public class DynamicResourceLoader : ComponentBase
    {
        ...
        {
#if SERVER
            {"pdf-viewer", new() { CssDirectory= "smartpdfviewer", JsFiles= new[] { "syncfusion-blazor-sfsmartpdfviewer", "sf-grid", "sf-uploader", "sf-multiselect", "sf-accordion", "sf-sidebar", "sf-dropdownlist", "sf-progressbar" } } },
#else
            {"pdf-viewer", new() { CssDirectory= "pdfviewer", JsFiles= new[] {"syncfusion-blazor-sfpdfviewer", "sf-grid", "sf-uploader", "sf-multiselect", "sf-accordion", "sf-sidebar", "sf-dropdownlist", "sf-progressbar" } } },
#endif
        ...
        private string GetResourcesMarkup(bool isAddScript)
        {
            ...
            var cdnScriptPath = "https://cdn.syncfusion.com/blazor/32.2.3/";
            var cdnStylePath = "https://cdn.syncfusion.com/blazor/documentprocessing-sb/pdfviewer/styles/32.2.3/";
            var demoPageCompsScript = new string[] { "sf-tab", "sf-tooltip", "sf-toast" };
            if (SampleUtils.IsHomePage(UriHelper))
            {
#if (NET8_0 && !WASM)
                sb.AppendLine($"<script src=\"{cdnScriptPath}sf-drop-down-button.min.js\" defer></script>");
#else
            ...
            else
            {
                ...
                {
#if (NET8_0 && !WASM)
                    sb.AppendLine($"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/documentprocessing-sb/pdfviewer/styles/32.2.3/demo-page-comps/{theme}.min.css\" />");
                  
#else
                    sb.AppendLine(CultureInfo.InvariantCulture, $"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/documentprocessing-sb/pdfviewer/styles/32.2.3/demo-page-comps/{theme}.min.css\" />");
#endif
                }
                ...
                if (SampleService?.SampleName == null)
                {
                    if (!string.IsNullOrEmpty(resources.CssDirectory))
                    {
                        ...
                        {
#if (NET8_0 && !WASM)
                            sb.AppendLine($"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/32.2.3/styles/{theme}.css\" />");
#else
                            sb.AppendLine(CultureInfo.InvariantCulture, $"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/32.2.3/styles/{theme}.css\" />");
#endif
                        }
                        else
                        {
#if (NET8_0 && !WASM)
                            sb.AppendLine($"<link rel=\"preload\" href=\"{cdnStylePath}{resources.CssDirectory}/{theme}.min.css\" as=\"style\"/>");
                            sb.AppendLine($"<link rel=\"stylesheet\" href=\"{cdnStylePath}{resources.CssDirectory}/{theme}.min.css\" />");
#else
                            sb.AppendLine(CultureInfo.InvariantCulture, $"<link rel=\"preload\" href=\"{cdnStylePath}{resources.CssDirectory}/{theme}.min.css\" as=\"style\"/>");
                    ...
                    if (resources.JsFiles != null && isAddScript == true)
                    {
                        foreach (var file in resources.JsFiles)
                        {
                           ...
                           sb.AppendLine(GetLoadScriptOnceMarkup(jsPath));
                ...
                else
                {
                    if (!string.IsNullOrEmpty(resources.CssDirectory))
                    {
                        if (resources.CssDirectory == "overall")
                        {
                            ...
                            sb.AppendLine($"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/32.2.3/styles/{theme}.css\" />");
#else
                            sb.AppendLine(CultureInfo.InvariantCulture, $"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/32.2.3/styles/{theme}.min.css\" />");
#endif
                        }
                ...
                {
#if (NET8_0)
                    sb.AppendLine($"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/documentprocessing-sb/pdfviewer/styles/32.2.3/common/highcontrast.min.css\" />");
                    sb.AppendLine($"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/documentprocessing-sb/pdfviewer/styles/32.2.3/common/dark-theme.min.css\" />");
#else
                    sb.AppendLine(CultureInfo.InvariantCulture, $"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/documentprocessing-sb/pdfviewer/styles/32.2.3/common/highcontrast.min.css\" />");
                    sb.AppendLine(CultureInfo.InvariantCulture, $"<link rel=\"stylesheet\" href=\"https://cdn.syncfusion.com/blazor/documentprocessing-sb/pdfviewer/styles/32.2.3/common/dark-theme.min.css\" />");
#endif
                }
                ...
                {
#if (NET8_0 && !WASM)
                    sb.AppendLine($"<script src=\"{cdnScriptPath}sf-tab.min.js\" defer></script>");
                    sb.AppendLine($"<script src=\"{cdnScriptPath}sf-tooltip.min.js\" defer></script>");
                    sb.AppendLine($"<script src=\"{cdnScriptPath}sf-toast.min.js\" defer></script>");
#else
                    sb.AppendLine(CultureInfo.InvariantCulture, $"<script src=\"{cdnScriptPath}sf-tab.min.js\" defer></script>");
                    sb.AppendLine(CultureInfo.InvariantCulture, $"<script src=\"{cdnScriptPath}sf-tooltip.min.js\" defer></script>");
                    sb.AppendLine(CultureInfo.InvariantCulture, $"<script src=\"{cdnScriptPath}sf-toast.min.js\" defer></script>");
#endif
                }
            }
            return sb.ToString();
        ...
        {
            string componentName = SampleService?.ComponentName != null && SampleService?.ComponentName != "PDF Viewer"? "smart-pdf-viewer": "pdf-viewer";
        ...
        private static string GetLoadScriptOnceMarkup(string scriptUrl)
        {
            ...
            (function () {{
                ...
                if (!window.loadScriptOnce) {{
                    window.loadScriptOnce = function (url) {{
                        if (!url || window._loadedScripts[url]) return;
                ...
                window.loadScriptOnce('{scriptUrl}');
            }})();
            </script>");
        }
    }
