    ...
    public class SampleMetaData : ComponentBase, IDisposable
    {
        ...
        private string GetContent()
        {
            ...
            if (SampleService?.ComponentName != null && SampleService.SampleInfo != null)
            {
                ...
                componentName = (SampleService.SampleInfo.Directory!).Contains("DocumentProcessing/", StringComparison.OrdinalIgnoreCase) ? componentName + " Library -" : componentName;
                var sampleInfo = SampleService.SampleInfo;
                var sampleName = sampleInfo.MappingSampleName != null && !UriHelper!.Uri.Contains(sampleInfo.Url!, StringComparison.OrdinalIgnoreCase) ? sampleInfo.MappingSampleName : sampleInfo.Name;
                sb.Append($"<title>");
            ...
            {
                var metaTitle = "Blazor PDF Viewer Components Examples & Demos | Syncfusion";
        ...
        private string RenderCanonicalTag()
        {
            ...
            if (canonicalUrl.Length > 0 && canonicalUrl[0].StartsWith("https://liveviewereditorblazorapp.azurewebsites.net", StringComparison.OrdinalIgnoreCase))
            {
                canonicalUrl[0] = canonicalUrl[0].Replace("https://liveviewereditorblazorapp.azurewebsites.net", "https://document.syncfusion.com", StringComparison.OrdinalIgnoreCase);
            }
#if (NET8_0 && !WASM)
            sb.Append($" href=\"{canonicalUrl[0]}\"");
#else
            sb.Append(CultureInfo.InvariantCulture, $" href=\"{canonicalUrl[0]}\"");
#endif
            sb.Append(" />");
            sb.Append(Environment.NewLine);
            return sb.ToString();
        ...
        private void AppendOpenGraphTags(StringBuilder sb, string title, string description, string image)
        {
            ...
            sb.AppendLine(CultureInfo.InvariantCulture, $"<meta property=\"og:type\" content=\"website\" />");
            sb.AppendLine(CultureInfo.InvariantCulture, $"<meta property=\"og:site_name\" content=\"Syncfusion Blazor Demos\" />");
        ...
        private void AppendTwitterCardTags(StringBuilder sb, string title, string description, string image)
        {
...
#else
            sb.AppendLine(CultureInfo.InvariantCulture, $"<meta name=\"twitter:account_id\" content=\"41152441\" />");
            ...
            sb.AppendLine(CultureInfo.InvariantCulture, $"<meta name=\"twitter:card\" content=\"summary\" />");
