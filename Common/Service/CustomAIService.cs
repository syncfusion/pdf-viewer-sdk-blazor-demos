    ...
    public class CustomAIService : IChatInferenceService
    {
        ...
        public async Task<string> GetCompletionAsync(string prompt, bool returnAsJson = true, bool appendPreviousResponse = false, string systemRole = null!, int outputTokens = 2000)
        {
            ...
            try
            {
                ...
                string completion = await GenerateResponseAsync(chatParameters).ConfigureAwait(true);
                if (appendPreviousResponse)
                {
        ...
        public async Task<string> GenerateResponseAsync(ChatParameters options)
        {
            if (options != null)
            {
                string userCode = await _userTokenService.GetUserFingerprintAsync().ConfigureAwait(true);
                int remainingTokens = await _userTokenService.GetRemainingTokensAsync(userCode).ConfigureAwait(true);
                int inputTokens = options.Messages!.Sum(message => message.Text.Length / 4);

                if (remainingTokens <= inputTokens)
                {
                    await _userTokenService.ShowAlert(userCode).ConfigureAwait(true);
                    return null!;
                }
                ...
                try
                {
                    ChatResponse completion = await _chatClient.GetResponseAsync(options.Messages!, completionRequest).ConfigureAwait(true);
                    await _userTokenService.UpdateTokensAsync(userCode, (int)(remainingTokens - completion.Usage!.TotalTokenCount)!).ConfigureAwait(true);
