...
namespace BlazorDemos.Controllers
{
    ...
    public class UserTokensController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRemainingTokens(string userId)
        {
            ...
            int remainingTokens = await userToken.GetRemainingTokensAsync(userId).ConfigureAwait(true);
            string alertMessage = await userToken.ReturnAlertMessage(userId).ConfigureAwait(true);
            if (remainingTokens <= 300)
            {
                return Ok(new { remainingTokens, alertMessage });
            }
            await userToken.UpdateTokensAsync(userId, (int)(remainingTokens - 550)).ConfigureAwait(true);
            return Ok(new { remainingTokens, alertMessage });
        }
    }
}
