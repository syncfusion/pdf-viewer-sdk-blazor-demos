...
namespace BlazorDemos.Pages.Viewer
{
    public partial class ProgrammaticalAnnotationBase
    {
        ...
        {
            if (!string.IsNullOrEmpty(annotation.DynamicText))
            {
            ...
            else
            {
                if (selectedAnnotation.SignHereStampList.Contains(annotation.Subject))
                {
                    ...
                    selectedAnnotation.SignHereStamp = annotation.Subject.Replace(" ", "", StringComparison.Ordinal);
                }
                else
                {
                    selectedAnnotation.StampsType = "StandardBusiness";
                    selectedAnnotation.StandardBusinessStamp = annotation.Subject.Replace(" ", "", StringComparison.Ordinal);
                }
            }
        }
        ...
        {
            char[] _rgbaSeparators = { '(', ',', ')' };
            string[] rgbaValues = rgba.Split(_rgbaSeparators, StringSplitOptions.RemoveEmptyEntries);
            int r = int.Parse(rgbaValues[1], NumberStyles.Integer, CultureInfo.InvariantCulture);
            int g = int.Parse(rgbaValues[2], NumberStyles.Integer, CultureInfo.InvariantCulture);
            int b = int.Parse(rgbaValues[3], NumberStyles.Integer, CultureInfo.InvariantCulture);
            int a = (int)(double.Parse(rgbaValues[4], NumberStyles.Float, CultureInfo.InvariantCulture) * 255);
            if (type == "stroke")
            {
        ...
        internal void ResetAnnotationProperties(AnnotationBase annotation)
        {
            annotation.VertexPoints = new List<VertexPoint>();
