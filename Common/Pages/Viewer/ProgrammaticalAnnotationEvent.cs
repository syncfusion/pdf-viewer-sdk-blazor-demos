...
namespace BlazorDemos.Pages.Viewer
{
    public partial class ProgrammaticalAnnotationBase
    {
        ...
        internal async void AddAnnotation()
        {
            if (PdfViewerInstance != null)
            {
                ...
                if (selectedAnnotation.AnnotationType != "Stamp")
                {
                    ...
                    await PdfViewerInstance.AddAnnotationAsync(newShapeAnnotation).ConfigureAwait(true);
                }
                else if (selectedAnnotation.StampsType == "Dynamic")
                {
                    DynamicStampItem dynamicAnnotationType = Enum.Parse<DynamicStampItem>(selectedAnnotation.DynamicStamp.Replace(" ", "", StringComparison.Ordinal));
                    //Adding the Dynamic Stamp Annotation in the PDF viewer
                    await PdfViewerInstance.AddAnnotationAsync(newShapeAnnotation, dynamicAnnotationType).ConfigureAwait(true);
                }
                else if (selectedAnnotation.StampsType == "SignHere")
                {
                    SignStampItem signAnnotationType = Enum.Parse<SignStampItem>(selectedAnnotation.SignHereStamp.Replace(" ", "", StringComparison.Ordinal));
                    //Adding the Sign Stamp Annotation in the PDF viewer
                    await PdfViewerInstance.AddAnnotationAsync(newShapeAnnotation, signAnnotationType).ConfigureAwait(true);
                }
                else if (selectedAnnotation.StampsType == "StandardBusiness")
                {
                    StandardBusinessStampItem standardBusinessAnnotationType = Enum.Parse<StandardBusinessStampItem>(selectedAnnotation.StandardBusinessStamp.Replace(" ", "", StringComparison.Ordinal));
                    //Adding the Standard Business Stamp Annotation in the PDF viewer
                    await PdfViewerInstance.AddAnnotationAsync(newShapeAnnotation, standardBusinessAnnotationType).ConfigureAwait(true);
                }
        ...
        internal async void UpdateAnnotation()
        {
            ...
            pdfAnnotationList = await PdfViewerInstance!.GetAnnotationsAsync().ConfigureAwait(true);
            ...
            if (existingAnnotationIndex != -1)
            {
                ...
                await PdfViewerInstance.EditAnnotationAsync(existingAnnotation).ConfigureAwait(true);
            }
        }
        ...
        internal async Task AnnotationSelectedEvent(AnnotationSelectEventArgs annotationSelectEventArgs)
        {
            ...
            selectedAnnotation.IsInputChange = false;
            await UpdatePropertiesInPropertyPanel(annotationSelectEventArgs.AnnotationId).ConfigureAwait(true);
        }
        ...
        internal void DocumentLoaded()
        {
            ...
            if (PdfViewerInstance != null && numericTextBoxRef != null)
            {
                ...
                PropertyInfo maxProp = numericTextBoxRef.GetType().GetProperty("Max")!;
                maxProp?.SetValue(numericTextBoxRef, PdfViewerInstance.PageCount);
                StateHasChanged();
            }
        }
        ...
        {
            selectedAnnotation.IsInputChange = true;
            if (string.IsNullOrEmpty(selectedAnnotation.Comment))
            {
            ...
            comment.ModifiedDate = DateTime.Now.ToString(CultureInfo.CurrentCulture);
        ...
        internal async void ImageChane(UploadChangeEventArgs action)
        {
            ...
            await action.Files[0].File.OpenReadStream(long.MaxValue).CopyToAsync(fileStream).ConfigureAwait(true);
        ...
        internal async Task OnFileRemove()
        {
            ...
            await uploadObj!.RemoveAsync().ConfigureAwait(true);
        }
        ...
        internal void LockChange(Microsoft.AspNetCore.Components.ChangeEventArgs changeEventArgs)
        {
            ...
            {
                string valueText = changeEventArgs.Value?.ToString()!;

                if (!string.IsNullOrWhiteSpace(valueText) && bool.TryParse(valueText, out var isLocked))
                {
                    selectedAnnotation.IsLocked = isLocked;
                }
            }
