    ...
    public partial class CustomToolBarBase
    {
        ...
        public async Task FileUploadSelected(UploadingEventArgs args)
        {
            if (args?.FileData.Type == "pdf")
            {
                ...
                await ViewerInstance!.LoadAsync(base64string, null!).ConfigureAwait(true);
                await uploadFiles!.ClearAllAsync().ConfigureAwait(true);
                if (module != null)
                {
                    await module.InvokeVoidAsync("changeFocus").ConfigureAwait(true);
                }
            }
        }
